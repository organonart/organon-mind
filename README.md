...
scripts/    two generators, running in opposite directions; see the warnings below
```
...
`doc/om-003.md` is a different kind of thing again: a **rendering** of a
publication, not a source for one. It exists because the paper gets read in
places that will not take HTML. It is generated by
`scripts/render_paper_markdown.py` and should never be hand-edited — regenerate
it instead:
...
**The direction is the whole point.** `generate_mind_why.py` produces a *page*,
so a hand edit to its output is lost at the next run. This one consumes a page
and produces something disposable: `site/om-003.html` stays canonical, the
markdown can be thrown away and rebuilt, and no generator can overwrite a
publication. A rendering that nobody can reproduce is a second copy waiting to
drift; this way a divergence is one command to fix rather than a diff to
reconcile by hand.

The figures do not survive the trip — they are hand-drawn inline SVG. Each
becomes its `aria-label`, which is written as a description of the diagram, plus
its caption. That is stated at the top of the rendering rather than left for a
reader to notice.
